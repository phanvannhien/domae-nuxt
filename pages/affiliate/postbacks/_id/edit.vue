                ...
                <el-col :sm="24" :md="12">
                   ...
                   <el-form-item label="Select params">
                        <el-checkbox-group v-model="form.params">
                            <el-checkbox disabled style="width: 200px" :label="item.param" v-for="item in tableData" :key="item.param"></el-checkbox>
                        </el-checkbox-group>
                    </el-form-item>
...
export default {
    ...
    async asyncData ({ $axios, params, req, res }) {
        ...
        return $axios.get(`/postbacks/${params.id}`)
            .then((res) => {
        ...
        onSubmit(formName){
            this.$refs[formName].validate( async (valid) => {
                if (valid) {
                    ...
                    this.$axios.post( `/postbacks/${ this.$route.params.id }`, this.form )
                        .then( res => { 
                            
                            this.$message({
                                message: 'Congrats, Bạn cập nhật thành công',
                                type: 'success'
                            });
